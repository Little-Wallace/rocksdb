...
void WriteBatchInternal::AsyncInsertInto(WriteThread::Writer* writer, SequenceNumber sequence,
        ...
        bool ignore_missing_column_families, DB* db, ThreadPool* pool) {
  auto write_group = writer->write_group;
  write_group->running.fetch_add(writer->batches.size(), std::memory_order_relaxed);
  for (auto w : writer->batches) {
    pool->SubmitJob([=]() {
        ColumnFamilyMemTablesImpl memtables(version_set);
        MemTableInserter inserter(
                sequence, &memtables, flush_scheduler, ignore_missing_column_families,
                0, db, true, nullptr /*has_valid_writes*/);
        inserter.set_log_number_ref(writer->log_ref);
